(() => {
  ...
  const FLAVOR_JSON = 'sabores.json?v=2';
  ...
  function injectStyles() {
    if (document.getElementById('liftFlavorStylesV2')) return;
    const style = document.createElement('style');
    style.id = 'liftFlavorStylesV2';
    style.textContent = `
      ...
      .flavorChoice{width:100%;min-height:46px;border:1px solid #ddd;background:#fff;border-radius:11px;padding:10px 12px;text-align:left;font-family:inherit;font-size:13px;font-weight:300;cursor:pointer}
  ...
  function injectPicker() {
    document.getElementById('flavors')?.remove();
  ...
  function openFlavorPicker(id, action = 'cart') {
    const product = PRODUCTS.find((x) => sameId(x.id, id));
  ...
  function closeFlavorPicker() {
    ...
    const anotherOpen = [...document.querySelectorAll('.off.open')].find((x) => x.id !== 'flavors');
  ...
  function overrideCatalogBehavior() {
    ...
    items = function () {
      return Object.entries(cart).map(([key, qty]) => {
        ...
        return {
          key,
          p: PRODUCTS.find((x) => sameId(x.id, parsed.id)),
          q: Number(qty || 0),
    ...
    add = function (id, flavor) {
      if (typeof flavor === 'string' && flavor) return rawAdd(id, flavor);
      if (flavorsFor(id).length) openFlavorPicker(id, 'cart');
      else rawAdd(id, '');
    };

    buyProduct = function (id) {
      if (flavorsFor(id).length) openFlavorPicker(id, 'buy');
    ...
    renderCart = function () {
      ...
      if (!cartItems.length) {
        ...
        body.querySelectorAll('[data-cart-key]').forEach((button) => button.addEventListener('click', () => change(button.dataset.cartKey, Number(button.dataset.delta || 0))));
        body.querySelectorAll('[data-remove-key]').forEach((button) => button.addEventListener('click', () => removeItem(button.dataset.removeKey)));
      }
    ...
    checkoutItemsList = function () {
      if (checkoutMode === 'single') {
        const product = PRODUCTS.find((x) => sameId(x.id, checkoutSingleId));
    ...
    renderCheckoutSummary = function () {
      ...
      document.getElementById('checkoutItems').innerHTML = checkoutItems.map((row) => `${row.q} × ${esc(row.p.name)}${row.flavor ? ` · Sabor: ${esc(row.flavor)}` : ''} · ${fmt(row.p.price * row.q)}`).join('<br>');
    ...
    const originalCheckoutText = checkoutText;
    checkoutText = function () {
      const text = originalCheckoutText();
      if (!text) return text;
      const checkoutItems = checkoutItemsList();
      if (!checkoutItems.some((x) => x.flavor)) return text;
      ...
      const lines = ['Hola, quiero confirmar este pedido:', '', `Nombre: ${name}`, `Celular: ${phone}`, `Dirección: ${address}`, `Ciudad: ${selectedCity.name}${selectedCity.department ? ' · ' + selectedCity.department : ''}`, notes ? `Notas: ${notes}` : '', '', 'Pedido:'];
      checkoutItems.forEach(({ p, q, flavor }) => lines.push(`⚡ ${q} ${p.name} – ${p.brand}${flavor ? ' · Sabor: ' + flavor : ''} · ${fmt(p.price)} c/u`));
    ...
    orderText = function () {
      ...
      const lines = ['Pedido:', ''];
      cartItems.forEach(({ p, q, flavor }) => lines.push(`⚡ ${q} ${p.name} – ${p.brand}${flavor ? ' · Sabor: ' + flavor : ''} · ${fmt(p.price)} c/u`));
  ...
  async function init() {
    ...
    try {
      ...
      console.info('Lift sabores cargados:', Object.keys(flavorData.products || {}).length);
    } catch (error) {
